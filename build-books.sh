...
if [[ $# -gt 0 && "${1:-}" != "--defaults" ]]; then
    ...
    echo "Run ./build-books.sh --help for usage." >&2
    exit 2
fi
...
if command -v pypy3 &>/dev/null && pypy3 -c "import chess; import chess.polyglot; import chess.pgn" 2>/dev/null; then
    ...
    if [[ ! -d "$VENV" ]]; then
        echo -e "  ${CYAN}Creating standalone Python environment at $VENV${RESET}"
        command -v python3 &>/dev/null || install_hint "python3"
        if ! python3 -m venv "$VENV"; then
            echo -e "${RED}✖${RESET} Could not create the Python environment."
            [[ "$OS" == "Darwin" ]] || echo "   Install python3-venv: sudo apt install python3-venv"
            exit 1
        fi
        "$VENV/bin/python" -m pip install --disable-pip-version-check "python-chess>=1.999,<2"
    fi
...
if [[ -f "$CHUNK" ]] && [[ $(file_size "$CHUNK") -gt $((MAX_BYTES / 2)) ]]; then
    ...
    curl -L \
        --range "0-$((MAX_BYTES - 1))" \
        --progress-bar \
        --user-agent "chess-opening-book-builder/1.0" \
        -o "$CHUNK" \
        "$DB_URL"
...
OUTPUT_DIR = os.path.expanduser(os.environ["BOOKS_DIR"])
...
export RATINGS SPEED_FILTER SPEED_NAME BOOKS_DIR
